...
On NLMT server a json file can be produced if used like this:
```
...
A `.json.gz` file will be created with an autogenerated name in `/tmp/edaf1/test/server` folder.

NLMT client running command won't change compared to the online EDAF.

After running the experiment you will have 3 files: 2 `*.lseq` and 1 `se_*.json.gz`.
